...
class _RemoteInputWorkspaceScreenState extends State<RemoteInputWorkspaceScreen>
  ...
  void dispose() {
    ...
    for (final timer in _layoutSaveTimers.values) {
      timer.cancel();
    }
    super.dispose();
  }
      ...
      body: SafeArea(
        child: _loading
            ...
            : RemoteInputAdaptiveWorkspace(
                ...
                detailsPanel: _buildDetailsPanel(l10n),
  ...
  RemoteInputScreenRect _peerLayoutBounds(
    ...
    return _boundsFor(
      _peerDisplaysForLayout(device, layout)
  ...
  void _movePeerLayoutByKey(
    DeviceData device,
    RemoteInputEdge direction,
    bool coarse,
  ) {
    final current = _layouts[device.uid];
    if (current == null) {
      return;
    }
    ...
    setState(() {
      _focusedPeerId = device.uid;
      _layouts[device.uid] = current.copyWith(x: moved.x, y: moved.y);
    });
    _layoutSaveTimers[device.uid]?.cancel();
    _layoutSaveTimers[device.uid] = Timer(
      const Duration(milliseconds: 160),
      () => unawaited(_persistKeyboardMovedLayout(device)),
    );
  }

  Future<void> _persistKeyboardMovedLayout(DeviceData device) async {
    _layoutSaveTimers.remove(device.uid);
    final layout = _layouts[device.uid];
    if (layout == null) {
      return;
    }
    final rect = RemoteInputScreenRect(
      x: layout.x,
      y: layout.y,
      width: layout.width,
      height: layout.height,
    );
    final layoutJson = _layoutJsonForSnappedLayout(layout, rect);
    final next = layout.copyWith(
      peerName: device.name,
      layoutVersion: layoutJson.isEmpty ? 1 : 2,
    ...
    final latest = _layouts[device.uid];
    if (latest?.x == next.x && latest?.y == next.y) {
      setState(() => _layouts[device.uid] = next);
  ...
  _WorkspaceSharingPlan? _topologySharingPlanForDevice(
    ...
    if (_socketManager.supportsRemoteInputTopologyFor(device.uid)) {
      final remoteTopology =
          _socketManager.remoteDisplayTopologyFor(device.uid);
      if (remoteTopology != null) {
        final remoteBounds = remoteTopology.virtualBounds;
        final savedLayout = layout.savedLayout ??
            RemoteInputLayoutGeometry.savedLayoutForTranslatedSinkTopology(
              sourceTopology: localTopology,
              sinkTopology: remoteTopology,
              sinkOffsetX: layout.x - remoteBounds.x,
              sinkOffsetY: layout.y - remoteBounds.y,
              preferredSinkDisplayId: remoteTopology.primaryDisplay.displayId,
  ...
  String _layoutJsonForSnappedLayout(
    ...
    final savedLayout =
      ...
      sinkTopology: remoteTopology,
      sinkOffsetX: snapped.x - remoteBounds.x,
      sinkOffsetY: snapped.y - remoteBounds.y,
