...
void main() {
  ...
  test('issues a canonical single-use token bound to the complete claim', () {
    ...
    expect(claim.mediaMacKey, orderedEquals(mediaKey));
    expect(
      registry.consume(
        route: '/audio',
        sessionId: 'audio-session',
  ...
  test('namespace revocation preserves another audio type sharing the tuple',
      () {
  ...
  test('peer and session revocation release claims without exposing key state',
      () {
    registry.issue(
      route: '/audio',
      sessionId: 'audio-a',
    ...
    final exposed = claim.mediaMacKey;
    exposed[0] ^= 0xff;
    expect(claim.mediaMacKey, orderedEquals(mediaKey));
  });

  test('constant-time byte comparison covers early, late, and length mismatch',
      () {
