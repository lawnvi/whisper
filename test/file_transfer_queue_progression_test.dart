...
void main() {
  ...
  test('ACK validates durable and actual-sent boundaries before mutation',
      () async {
  ...
  test('duplicate READY at the current offset preserves the in-flight window',
      () async {
    final database = LocalDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    ...
    final ready = _controlFrame(_control(
      FileTransferV3Action.ready,
      _firstId,
      size: 4,
      offset: 0,
  ...
  test('delayed READY below committed progress cannot rewind active transfer',
      () async {
    final database = LocalDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    await _persistOutgoing(database, _firstId, size: 4);
  ...
  test('ACK from another connection generation cannot cancel the window',
      () async {
    final database = LocalDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    await _persistOutgoing(database, _firstId, size: 4);
  ...
  test('watchdog retransmits two durable windows then removes one generation',
      () async {
    final database = LocalDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    await _persistOutgoing(database, _firstId, size: 4);
  ...
  test('duplicate committed receiver data only re-ACKs durable progress',
      () async {
  ...
  for (final action in <FileTransferV3Action>[
    ...
    test('remote $action releases once and starts one outgoing successor',
        () async {
      final database = LocalDatabase.forTesting(NativeDatabase.memory());
      addTearDown(database.close);
  ...
  test('queued successor accepts a 2 MiB ACK while its window is still sending',
      () async {
    ...
    expect(
      successorData.map((frame) => frame.offset),
      <int>[
        0,
    ...
    expect(successorData.map((frame) => frame.sequence), <int>[0, 1, 2, 3, 4]);
    expect(
      (await database.fetchFileTransfer(_secondId))?.committedBytes,
      fileTransferV3AckIntervalSize,
    );
    expect(watchdog.currentWindow(_secondId)?.durableOffset,
        fileTransferV3AckIntervalSize);
    expect(watchdog.currentWindow(_secondId)?.sentEnd, successorSize);
  });

  test('source read throw releases active and starts its queued successor',
      () async {
    final database = LocalDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    for (final id in <String>[_firstId, _secondId]) {
  ...
  for (final throws in <bool>[false, true]) {
    test('generation-bound send ${throws ? 'throw' : 'false'} progresses queue',
        () async {
      final database = LocalDatabase.forTesting(NativeDatabase.memory());
      addTearDown(database.close);
      for (final id in <String>[_firstId, _secondId]) {
  ...
  for (final action in <FileTransferV3Action>[
    ...
    test('remote $action releases once and starts one incoming successor',
        () async {
  ...
  test('queued incoming cancel leaves active and skips to the next valid item',
      () async {
    final root = await Directory.systemTemp.createTemp('whisper-incoming-');
    addTearDown(() => root.delete(recursive: true));
    final database = LocalDatabase.forTesting(NativeDatabase.memory());
  ...
  test('disconnect pauses without progression and resume starts at committed',
      () async {
    final database = LocalDatabase.forTesting(NativeDatabase.memory());
    addTearDown(database.close);
    await _persistOutgoing(database, _firstId, size: 4);
  ...
  test('incoming successor loop persistently skips every stale queue shape',
      () async {
    final root = await Directory.systemTemp.createTemp('whisper-incoming-');
    addTearDown(() => root.delete(recursive: true));
    ...
    expect((await database.fetchFileTransfer(_thirdId))?.state,
        FileTransferState.canceled);
    expect((await database.fetchFileTransfer(_fourthId))?.state,
        FileTransferState.failed);
    expect((await database.fetchFileTransfer(_fourthId))?.lastError,
        'stale_queue');
    expect((await database.fetchFileTransfer(_fifthId))?.state,
        FileTransferState.failed);
    expect(
        (await database.fetchFileTransfer(_fifthId))?.lastError, 'stale_queue');
...
Iterable<WhisperFrameV3> _dataFrames(
  ...
  String transferId,
    ...
    frames.where(
      (frame) =>
          frame.type == WhisperFrameType.fileData &&
