...
void main() {
  ...
  group('AudioGroupCoordinator', () {
    ...
    test('source creates one group offer per sink with shared stream', () {
      ...
      expect(sent.map((item) => item.control.streamId).toSet(), <String>{
        'stream-1',
      });
    ...
    test(
      'source starts capture once and fans packets out to accepted sinks',
      () async {
        final sent = <_SentGroupControl>[];
    ...
    test(
      'accept completing after group stop closes its stale transport',
      () async {
    ...
    test(
      'capture startup completing after group stop stays cancelled',
      () async {
        ...
        expect(
          ...
          isNot(contains('startCapture')),
        );
      },
    );

    test(
      'group stop waits for pending native capture start then stops it',
      () async {
    ...
    test(
      'last failed group transport stops source capture exactly once',
      () async {
    ...
    test(
      'intentional group stop does not mark observable sink failed',
      () async {
        final transport = _ObservableFakeAudioGroupTransport();
        final coordinator = AudioGroupCoordinator(
          platform: platform,
    ...
    test(
      'source adapts group latency from the slowest synchronized sink',
      () async {
        final transports = <String, _FakeAudioGroupTransport>{};
        var now = 1000;
        final coordinator = AudioGroupCoordinator(
    ...
    test(
      'stopGroup sends stop to every sink and clears local session',
      () async {
        final sent = <_SentGroupControl>[];
        final coordinator = AudioGroupCoordinator(
          groupIdFactory: () => 'group-1',
    ...
    test(
      'pausePlaybackAsSink sends groupStop and keeps rejoin context',
      () async {
        final sent = <_SentGroupControl>[];
        final coordinator = AudioGroupCoordinator(
          platform: platform,
    ...
    test(
      'disconnect during pending rejoin declines the crossing offer',
      () async {
        final sent = <_SentGroupControl>[];
        final coordinator = AudioGroupCoordinator(
          platform: platform,
    ...
    test(
      'requestRejoinAsSink sends sinkJoinRequest with saved context',
      () async {
    ...
    test(
      'paused sink accepts a fresh offer without occupying the group',
      () async {
        final sent = <_SentGroupControl>[];
        final coordinator = AudioGroupCoordinator(
          platform: platform,
    ...
    test(
      'disconnectPlaybackAsSink sends groupStop and clears playback state',
      () async {
        final sent = <_SentGroupControl>[];
        final coordinator = AudioGroupCoordinator(
          platform: platform,
    ...
    test('sinkJoinRequest does not re-offer other terminal sinks', () async {
      ...
      expect(coordinator.session?.sinks['phone-a']?.lastError, 'remoteFailure');
      expect(
    ...
    test(
      'source turns clock report into sink timing metrics and correction',
      () async {
        final sent = <_SentGroupControl>[];
        var now = 1000000;
        final coordinator = AudioGroupCoordinator(
    ...
    test(
      'source retries clock probing when a preflight sample is congested',
      () async {
        final sent = <_SentGroupControl>[];
        var now = 1000000;
        final coordinator = AudioGroupCoordinator(
    ...
    test(
      'sink answers clock probe with local receive and send timestamps',
      () async {
        final sent = <_SentGroupControl>[];
        var now = 1100000;
        final coordinator = AudioGroupCoordinator(
    ...
    test(
      'sink reports playback evidence after receiving audio packet',
      () async {
        final sent = <_SentGroupControl>[];
        var now = 1000;
        final coordinator = AudioGroupCoordinator(
        ...
        expect(
          sent.single.control.action,
          AudioGroupControlAction.latencyReport,
        );
    ...
    test(
      'group packet decode failures throw synchronously to the channel',
      () async {
        final coordinator = AudioGroupCoordinator(
          platform: platform,
          codecFactory: _pcmCodec,
    ...
    test(
      'sink packet handling is not blocked by a slow native audio write',
      () async {
        ...
        final packetFuture = coordinator.handlePacket(
          AudioGroupPacketFrame(
            groupId: 'group-1',
            streamId: 'stream-1',
            sessionId: 'session-phone',
