...
void main() {
  group('FileTransferV3Metadata', () {
    ...
    test('rejects invalid size, digest, algorithm, chunk, and window', () {
      final cases = <({int size, Map<String, Object> json, String reason})>[
        ...
        (
          size: 1,
          json: <String, Object>{...validJson(), 'checksumAlgorithm': 'none'},
          reason: 'invalid_metadata',
        ),
        (
          size: 1,
          json: <String, Object>{...validJson(), 'checksumValue': 'abc123'},
          reason: 'invalid_metadata',
        ),
        (
          size: 1,
        ...
        (
          size: 1,
          json: <String, Object>{...validJson(), 'chunkSize': 0},
          reason: 'invalid_metadata',
        ),
        (
          size: 1,
        ...
        (
          ...
          json: <String, Object>{
            ...validJson(),
            'windowSize':
                fileTransferV3MaxOfferWindowSize +
  ...
  group('resume proof behavior', () {
    ...
    test(
      'sender accepts a matching proof and rejects a mismatched prefix',
      () async {
    ...
    test('receiver resets a mismatched prefix once and then fails', () async {
      ...
      expect((await database.fetchFileTransfer(transferId))?.committedBytes, 0);
      expect(
    ...
    test(
      'a recreated engine completes a persisted pending proof reset',
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'whisper-reset-recovery-',
        );
        addTearDown(() => directory.delete(recursive: true));
        final temp = File(await safeTransferTempPath(directory, transferId));
        await temp.parent.create(recursive: true);
    ...
    test(
      'reset recovery survives failure after filesystem truncation',
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'whisper-reset-fault-',
        );
        ...
        expect(pending?.resumeProofResetCount, pendingResumeProofResetMarker);
    ...
    test(
      'reset recovery survives a real filesystem failure after claim',
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'whisper-reset-fs-fault-',
        );
        addTearDown(() => directory.delete(recursive: true));
        final temp = File(await safeTransferTempPath(directory, transferId));
        await temp.parent.create(recursive: true);
    ...
    test(
      'receiver rejects zero, stale, or non-durable mismatch offsets',
      () async {
  ...
  group('incoming integrity and publication', () {
    ...
    test(
      'one-byte corruption fails integrity and deletes untrusted temp',
      () async {
        ...
        final tempPath = (await database.fetchFileTransfer(
          transferId,
        ))!.tempPath;
        await engine.handleFrame(
          _connection('peer-a'),
    ...
    test(
      'candidate symlink replacement is rejected before the next write',
      () async {
    ...
    test(
      'matching tail proof never replaces the final full-file SHA',
      () async {
        ...
        final engine = _engine(
          ...
          notifyFileVisible: (_) async =>
              fail('corrupt prefix must not publish'),
        );
    ...
    test(
      'zero-byte file verifies and publishes without a data frame',
      () async {
    ...
    test(
      'timestamp failure removes only the reservation and retains temp',
      () async {
    ...
    test(
      'visibility failure cannot block complete control or terminal cleanup',
      () async {
        const transferId = '61234567-89ab-4cde-8fab-0123456789ab';
        final root = await Directory.systemTemp.createTemp(
          'whisper-notify-failure-',
        );
        addTearDown(() => root.delete(recursive: true));
        final database = LocalDatabase.forTesting(NativeDatabase.memory());
        addTearDown(database.close);
  ...
  group('incoming offer admission', () {
    test(
      'invalid offer fields return stable reasons without persistence',
      () async {
    ...
    test(
      ...
      () async {
        ...
        for (
          var index = 0;
          index < maxNonterminalTransfersPerPeer;
          index += 1
        ) {
          final id =
    ...
    test(
      'stale session completes the DB invariant and duplicate resumes later',
      () async {
        const transferId = '51234567-89ab-4cde-8fab-0123456789ab';
        final root = await Directory.systemTemp.createTemp(
          'whisper-slow-offer-',
    ...
    test(
      'a duplicate offer after message deletion returns a stable error',
      () async {
        const transferId = '91234567-89ab-4cde-8fab-0123456789ab';
        final root = await Directory.systemTemp.createTemp(
          'whisper-deleted-offer-',
        );
        addTearDown(() => root.delete(recursive: true));
        final database = LocalDatabase.forTesting(NativeDatabase.memory());
        addTearDown(database.close);
  ...
  group('failure privacy', () {
    test(
      'remote detail never reaches persistence, snapshots, or notices',
      () async {
        const transferId = '81234567-89ab-4cde-8fab-0123456789ab';
        const secret =
            'token=never-log-this /Users/alice/Documents/private.txt';
        final database = LocalDatabase.forTesting(NativeDatabase.memory());
        addTearDown(database.close);
        ...
        expect(
          snapshots.last.lastError,
          FileTransferFailureReason.source.wireCode,
        );
...
final class _ClaimFaultDatabase extends LocalDatabase {
  _ClaimFaultDatabase(QueryExecutor executor, {required this.onClaimed})
    : super.forTesting(executor);
