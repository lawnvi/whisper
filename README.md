...
- **传输助手**：支持文件夹、全文搜索、收藏文本和默认关闭的剪贴板自动同步；同步内容不会进入会话记录。
...
- 多设备键鼠工作区支持双向共享剪贴板；最后一次复制生效，文本、图片和文件均不进入会话记录。
- 文件和图片先同步信息，目标设备执行粘贴时才传输内容，并受「自动同步剪贴板」开关和批次大小限制控制。
- 会话时间按间隔分组显示，复制按钮增加成功反馈；图片改为无标题全屏预览，Android 发出的媒体也可直接查看。
- 桌面端收到连接请求时会主动显示到前台，验证码弹窗状态切换更稳定。
- 清理旧的单设备键鼠排列设置，并修复 macOS Caps Lock 在被控端切换输入法需要按两次的问题。
