...
控制端与被控端都可以主动停止共享；目标断开后会释放对应输入控制。单设备共享与多设备工作区共用本机控制权，正在被控制的设备不能同时发起另一组共享，切换角色前请先停止当前共享。

需要跨设备粘贴时，开启自动同步剪贴板；复制文本、图片或文件后，在目标设备粘贴。截图也可以粘贴到 Whisper 输入框，确认内容后再发送；较大的图片或文件需要等待局域网传输完成。

macOS 请按提示授予辅助功能等所需权限；Linux 键鼠共享当前需要 X11，不能将 Wayland 下的截图支持视为键鼠共享支持。

### 截图并粘贴

使用桌面截图按钮或设置中的截图快捷键，点击选中窗口，或拖动选择区域，确认后图片会复制到剪贴板；按 `Esc` 取消。截图不会自动发送，在 Whisper 输入框中粘贴后可预览并发送。Linux 的可用能力取决于桌面截图服务，系统截图对话框的交互可能不同。
