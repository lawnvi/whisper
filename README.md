...
- **聊天式传输**：文本、剪贴板和文件都在会话里完成；图片可全屏查看，视频和音频可直接播放，并支持多选删除聊天记录。
...
- 图片、视频和音频消息加入即时预览与播放，接收中、校验中和完成状态会在同一张消息卡片内更新。
...
打开一个已连接设备的会话，直接输入文本并发送；点击附件按钮选择文件。图片可以点开全屏查看，视频和音频可以在消息中播放，其他文件会交给系统可用应用打开。长按消息并选择「多选」可以批量删除聊天记录；桌面端收到的文件还可以在文件管理器中定位，或从消息里拖出完成复制。
