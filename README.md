...
- **系统快捷发送**：支持 Android 分享菜单、桌面右键入口和全局快捷键，离线内容会保留到目标设备重连。
...
如果自动发现不可用，可以扫描对方显示的配对二维码，或手动输入局域网 IP 和服务端口。默认服务端口是 `10002`，也可以在设置里的「服务端口」修改。首次配对需要双方核对并确认同一组配对码。
...
- Linux：Flutter Linux desktop 依赖、Avahi、PulseAudio 或 PipeWire Pulse、libsecret/keybinder/jsoncpp 与可用的系统密钥环
...
- 配对后的业务数据使用端到端加密；局域网发现与连接元数据仍可见，本地数据库和暂存文件不做静态加密，当前实现也未经独立密码学审计。
