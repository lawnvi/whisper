...
- 提升局域网文件传输吞吐，并让大文件 SHA-256 校验跟随接收流完成，减少传输结束后的等待。
- 断线传输统一由发送端手动续传，接收端只显示中断状态，避免双方操作冲突。
- 增加 GitHub 应用内更新，桌面端安装更新前会安全退出当前进程。
- macOS 分别提供 Apple 芯片和 Intel 安装包，并修复跨架构运行及远程 Caps Lock 切换问题。
- 优化移动端全屏图片预览和图片切换动画，Android 连接请求仅在锁屏时额外通知。
